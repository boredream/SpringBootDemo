...
import com.boredream.springbootdemo.utils.StringUtils;
...
public class CodeService {
    ...
    public Map<String, String> genCode(String device, String doc) throws ApiException {
        TreeMap<String, ArrayList<ApiField>> apiParamsMap = parseApiCode(doc);

        if ("Android".equalsIgnoreCase(device)) {
            genAndroidClass(apiParamsMap);
        } else if ("iOS".equalsIgnoreCase(device)) {
            genIosClass(apiParamsMap);
        } else {
            throw new ApiException("平台暂不支持");
        }
    ...
    // 需要打印的代码
    private TreeMap<String, String> titleCodeMap = new TreeMap<>();
    ...
    // 解析接口数据
    private TreeMap<String, ArrayList<ApiField>> parseApiCode(String doc) {
    ...
    // 生成实体类
    private void genAndroidClass(TreeMap<String, ArrayList<ApiField>> apiParamsMap) {
        for (Map.Entry<String, ArrayList<ApiField>> entry : apiParamsMap.entrySet()) {
            ...
            titleCodeMap.put(className + ".java", sbClass.toString());
        }
    }

    // 生成 iOS实体类
    private void genIosClass(TreeMap<String, ArrayList<ApiField>> apiParamsMap) {
        StringBuilder sbClass = new StringBuilder();
        for (Map.Entry<String, ArrayList<ApiField>> entry : apiParamsMap.entrySet()) {
            String className = entry.getKey();
            ...
            titleCodeMap.put(className + ".swift", sbClass.toString());
        }
    }
